...
import {
  computeSaju, computeDaeun, pillarText, STEMS, BRANCHES, type Element,
} from '@/lib/saju/calc';
import { DAY_STEM_JA, TEN_GOD_JA, ELEMENT_LUCK, STRENGTH_JA } from '@/lib/saju/text';
...
{"headline":"20字以内の一言","personality":"性格を150字程度","strength":"強みを100字程度","caution":"気をつける点を100字程度","advice":"今日から出来る一歩を100字程度"}`;
...
export async function POST(req: NextRequest) {
  ...
  const cacheKey = crypto.createHash('sha256').update(
    `saju|${meishiki.year}${meishiki.month}${meishiki.day}${meishiki.hour ?? '-'}|${isMale ? 'm' : 'f'}|${personaKey}`
  ).digest('hex');
  ...
  if (!reading) {
    const elems = Object.entries(saju.elementCount)
      .map(([k, v]) => `${k}:${v}`).join(' ');
    const context = `【命式】年柱${meishiki.year} 月柱${meishiki.month} 日柱${meishiki.day} 時柱${meishiki.hour ?? '不明'}
...
${saju.hourUnknown ? '※出生時刻不明のため時柱なし。時柱に触れないこと。' : ''}`;
  ...
  return NextResponse.json({
    date: dateStr,
    meishiki,
    dayStem: { char: STEMS[saju.dayStem], ...dayStemInfo },
